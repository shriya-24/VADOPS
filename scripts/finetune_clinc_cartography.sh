...
# Argument 3(cartography_split): Specify the dataset split from ['train', 'validation']
# Argument 4(log_training_dynamics_dir): specify the dir path to store the training dynamics
python ../models/CLINC.py finetune $1 $2 True $3 $4

# Example
# Using python: python ../models/CLINC.py finetune {clinc_subset} /path/to/checkpoint True {cartography_split} {log_training_dynamics_dir}
# Using python: python ../models/CLINC.py finetune small ../checkpoints/clinc_small True train ../training_dynamics/clinc_small

# Using bash: sbatch finetune_clinc_cartography.sh {clinc_subset} /path/to/checkpoint {cartography_split} {log_training_dynamics_dir}
